...
func (s *triggerSubscriber) registrationLoop() {
	...
	for {
		...
		case <-ticker.C:
			...
			s.mu.RLock()
			s.lggr.Debugw("register trigger for remote capability", "donId", cfg.capDonInfo.ID, "nMembers", len(cfg.capDonInfo.Members), "nWorkflows", len(s.registeredWorkflows))
...
func (s *triggerSubscriber) Receive(_ context.Context, msg *types.MessageBody) {
	...
	case types.MethodTriggerEvent:
		...
		for idx, workflowID := range meta.WorkflowIds {
			...
			if ready {
				...
				s.lggr.Debugw("remote trigger event aggregated", "triggerEventID", meta.TriggerEventId, "workflowId", workflowID, "triggerID", triggerID)
	...
	default:
		s.lggr.Warnw("received trigger event with unknown method", "method", SanitizeLogString(msg.Method), "sender", sender, "err", SanitizeLogString(msg.ErrorMsg))
	}
}
...
func (s *triggerSubscriber) SetConfig(config *commoncap.RemoteTriggerConfig, capInfo commoncap.CapabilityInfo, localDONID uint32, remoteDON commoncap.DON, aggregator types.Aggregator) error {
	if config == nil {
		s.lggr.Warn("SetConfig called with nil config, using defaults")
