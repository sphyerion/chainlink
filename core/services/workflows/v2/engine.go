...
func (e *Engine) startExecution(ctx context.Context, wrappedTriggerEvent enqueuedTriggerEvent) {
	...
	if meteringErr != nil {
		lggr.Warnw("could start metering workflow execution. continuing without metering", "err", meteringErr)
	}
	...
	if execErr != nil {
		...
		executionLogger.Warnw("Workflow execution failed with module execution error", "status", executionStatus, "durationMs", executionDuration.Milliseconds(), "err", execErr)
		return
	}
	...
	if len(result.GetError()) > 0 {
		...
		executionLogger.Warnw("Workflow execution failed", "status", executionStatus, "durationMs", executionDuration.Milliseconds(), "error", result.GetError())
		return
	}
	...
	for {
		...
		case logLine, ok := <-userLogChan:
			...
			if err := events.EmitUserLogs(ctx, executionLabels, []*protoevents.LogLine{logLine}, executionID); err != nil {
				e.logger().Warnw("Failed to emit user logs", "err", err)
			}
			count++
		}
	}
