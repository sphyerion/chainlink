...
func (e *Engine) finishExecution(ctx context.Context, cma custmsg.MessageEmitter, executionID string, status string) error {
	...
	if err != nil {
		e.logger.Warnf("failed to emit execution finished event: %+v", err)
	}
...
func (e *Engine) worker(ctx context.Context) {
	...
	for {
		...
		case resp, isOpen := <-e.triggerEvents:
			...
			if resp.Err != nil {
				e.logger.Warnf("trigger event was an error %v; not executing", resp.Err)
			...
			if !senderAllowed {
				...
				e.logger.With(platform.KeyWorkflowID, e.workflow.id, platform.KeyWorkflowOwner, e.workflow.owner, platform.KeyWorkflowExecutionID, executionID).Warnf("failed to start execution: per sender rate limit exceeded")
			...
			if !globalAllowed {
				e.onRateLimit(executionID)
				e.logger.With(platform.KeyWorkflowID, e.workflow.id, platform.KeyWorkflowOwner, e.workflow.owner, platform.KeyWorkflowExecutionID, executionID).Warnf("failed to start execution: global rate limit exceeded")
...
func (e *Engine) workerForStepRequest(ctx context.Context, msg stepRequest) {
	...
	if !meteringOK {
		...
		l.Debugf("no metering report found for %v", msg.state.ExecutionID)
	}
