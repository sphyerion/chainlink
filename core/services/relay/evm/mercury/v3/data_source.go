	...
	if len(ds.jb.OCR2OracleSpec.PluginConfig) == 0 {
		...
		go func() {
			...
			if obs.LinkPrice.Val == nil && obs.LinkPrice.Err == nil {
				...
				ds.lggr.Warnw("Mercury server returned error querying LINK price feed", "err", obs.LinkPrice.Err, "linkFeedID", ds.linkFeedID)
			}
		}()
	}
	...
	if len(ds.jb.OCR2OracleSpec.PluginConfig) == 0 {
		...
		go func() {
			...
			if obs.NativePrice.Val == nil && obs.NativePrice.Err == nil {
				...
				ds.lggr.Warnw("Mercury server returned error querying native price feed", "err", obs.NativePrice.Err, "nativeFeedID", ds.nativeFeedID)
			}
		}()
	}
