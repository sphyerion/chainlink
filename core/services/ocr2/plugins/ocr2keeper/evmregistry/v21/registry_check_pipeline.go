...
func (r *EvmRegistry) simulatePerformUpkeeps(ctx context.Context, checkResults []ocr2keepers.CheckResult) ([]ocr2keepers.CheckResult, error) {
	...
	for i, cr := range checkResults {
		...
		if err != nil {
			...
			r.lggr.Warnw("failed get offchain config, gas price check will be disabled", "err", err, "upkeepId", upkeepId, "block", block)
		}
	...
	for i, req := range performReqs {
		...
		if !simulatePerformSuccess {
			r.lggr.Debugf("upkeepId %s is not eligible after simulation of perform", checkResults[idx].UpkeepID.String())
