...
func (c *LogProvider) StaleReportLogs(ctx context.Context) ([]ocr2keepers.StaleReportLog, error) {
	...
	for _, r := range reorged {
		...
		if err != nil {
			c.logger.Warn("error while fetching checkBlockNumber from reorged report log: %w", err)
			continue
		}
	...
	for _, r := range insufficientFunds {
		...
		checkBlockNumber, err := c.getCheckBlockNumberFromTxHash(ctx, r.TxHash, upkeepId)
		if err != nil {
			c.logger.Warn("error while fetching checkBlockNumber from insufficient funds report log: %w", err)
			continue
		}
		l := ocr2keepers.StaleReportLog{
			Key:             encoding.BasicEncoder{}.MakeUpkeepKey(checkBlockNumber, upkeepId),
